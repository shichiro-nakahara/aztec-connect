...
import { Blockchain, TxHash, TxType } from '@polyaztec/barretenberg/blockchain';
...
import { RollupDao, TxDao } from './entity/index.js';
...
describe('world_state', () => {
  ...
  beforeEach(() => {
    ...
    rollupDb = {
      ...
      getNextRollupId: jest.fn(() => getNextRollupId()),
      ...
      addRollup: jest.fn((rollupDao: RollupDao) => {
      ...
      getRollup: jest.fn((id: number) => rollupStore[id]),
    ...
    worldStateDb = {
      ...
      put: jest.fn(),
      getRoot: jest.fn((id: RollupTreeId) => roots[id]),
      getSubtreeRoot: jest.fn((id: RollupTreeId) => subTreeRoots[id]),
    ...
    blockchain = new (class extends EventEmitter {
      ...
      getUserPendingDeposit = jest.fn((asset: number, owner: EthAddress) => {
    ...
    pipeline = {
      getProcessedTxs: jest.fn(() => processedTxs),
      start: jest.fn(async () => {}),
      stop: jest.fn(async () => {}),
