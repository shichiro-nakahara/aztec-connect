...
export class FeeCalculator {
  constructor(
    ...
    private readonly debug = createDebugLogger('FeeCalculator')
  ) {
    this.log('Creating...');
      ...
      return [
        {
          assetId: feeAssetId,
          // AC SUNSET CODE
          value: (nextRollupFee * multiplier) / BigInt(multiplierPrecision)
        },
        {
          assetId: feeAssetId,
          value: (instantFee * multiplier) / BigInt(multiplierPrecision)
        },
      ]
    });
  }
  ...
  public getAliasFee(txAssetId: number, aliasLength: number) {
    ...
    const { aliasFee } = configurator.getConfVars().runtimeConfig;
